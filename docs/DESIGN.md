...
- Six districts (`DISTRICTS`), each with a Smile Index 0..1 starting at `start` (the city starts around 19%). The city index is the population-weighted mean. `state.city` holds each district's index and patients served.
...
- **Gains:** every patient served at a location raises that district's index by `(quality - 0.5) * 2 / population * cityGainMult` (so a poor clean below 0.5 lowers it); 80% of a location's patients come from its own district and 20% from a random neighbour, credited there. A wait walkout costs -0.5 points, a comfort walkout -1. The Smile Van (equipment, unlocked at 50%) serves the least-smiling district with no location: 6 free outreach cleanings a day at quality 0.8. Charity, school day and tooth fairy events add points to their location's district. Hands-on cleans count double (the owner's personal touch).
...
- **Milestones** (`CITY_MILESTONES`, every 10% from 30): a celebration card with the milestone text, a timeline entry, a cash gift (the city grant, scaled by tier) and the listed unlock (50%: Smile Van for sale; 70%: Golden Molar nomination; 90%: the gala is planned). 100%: the parade, then the gala is scheduled.
- **Pacing:** the median bot reaches 100% city at about 8 to 10 h of play on Standard (Relaxed about 6 to 7 h, Veteran 11 to 13 h). The balance tool reports it.
