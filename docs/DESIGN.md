...
The v3 targets in 10.8 replace this table where they differ. Since v4 (DESIGN 11) the v3 targets are the **Relaxed** targets (Relaxed plays the v3 rules), Standard owner milestones run about 20% longer, and the run ends with the Golden Molar Gala instead of the Floss Boss title (Smile City 100% at 6 to 7 h Relaxed, 8 to 10 h Standard, 11 to 13 h Veteran). Last `npm run balance` (median of 5 seeds, every bot on Standard plus the median bot on Relaxed and Veteran, full output in `out/sim/balance-v4.txt`): the table and checks are in 11.7 (the v3 run is kept in 10.8).
...
- **Whitening**: enamel starts at `startShade` (a shade guide from 1 brightest to 16 darkest). Polish the stains on the front teeth. **Paint gel on the front teeth (n/12)**: only arch positions 4 to 9 of each arch take gel (12 teeth, missing ones skipped, n counts the present ones). While a front tooth still needs gel it has a pulsing purple outline (brighter with the Gel brush in hand) and a purple mark on the mini-map; picking the Gel brush the first time shows "Gel goes on the front teeth: the ones glowing purple", and brushing any other tooth applies nothing and shows "Front teeth only" once. The gel is a glossy, slightly translucent violet coat (`#8B5CF6` family; the brush tip, its wrap ring and the Gel slot icon share it). A tooth snaps to a full, even coat once 60% of its outward face is painted (about 0.25 s of a brushing swipe, 0.5 s at most): a small squish and a gloss pop, and its outline goes. **Cure with the UV lamp**: the beam covers 4 neighbouring teeth of one arch (the aimed tooth, both neighbours and the next one on the side the beam centre is on, leaning back inward at the end of the front row); each gel-coated tooth under it gains one shade every 0.3 s, and a tooth 2+ shades behind the brightest front tooth of its arch catches up up to 1.6x faster, so the row finishes together. A typical cure of 6 to 8 shades on 12 teeth takes 8 to 12 s of lamp time (GPU pacing bot sweeping each arch at hand speed: 10.2 to 12.3 s to the goal at 1440x900 and 1024x768 touch). Every step pops: a quick brightness flash on that tooth, `shade_tick` rising in pitch with the whole smile's progress toward `targetShade`, and the shade meter bumps. More than 1.5 s nonstop aimed at one tooth zings (comfort -5, "Ooh, cold"), so keep the lamp moving. Reaching the goal plays the **Hollywood white** finale: a flash and sparkle wave across the front teeth left to right, `perfect` then `tooth_done`, and the meter lights up. Rinse off the gel (the shine reveal plays on every washed tooth) and suction. `shadeGain` = shades gained. Owner phase: needs the Whitening Lamp in that operatory, otherwise the visit is routine.
...
- In chair: NPC duration `d = minutes(service + addons) * (1.3 - 0.6*speed/100) * difficulty(archetype) * (assistant ? 0.8 : 1) * (ultrasonic kits ? 0.9 : 1)`, and at your own offices `* sqrt(dirtScale)` of the difficulty (v4: heavier mouths take your team longer too, Standard +5%, Veteran +14%).
...
lambda         = baseDemand(tier) * awareness * ratingFactor * priceFactor * marketing * (marketer 1.12) * equipment
                 ...
                 * Famous Name 1.1 (v4 Legacy perk, your locations)
...
- **Raise requests** (owner playtest: "too much work to give them raises every day"; the v3 rules asked about 0.9 times a day at a new Main Street Office with 6 staff). A level-up, a finished course or a skill event that raises the ask puts a request in the pipeline (`raiseDue`); it is settled at the day close, never mid-day:
  ...
  - **Raise all** (`payrollDay` skill, Management, level 4; `sim.raiseAllQuote`, `sim.raiseAll`): one tap raises every staff member paid below their ask at a location (or every location at once with `'all'`) up to their ask: salary = ask, morale +5, any pending raise request cleared, 10-day quiet period restarted, same bookkeeping as a manual raise. Temporary staff are skipped; staff already at or above their ask are untouched. Nobody below their ask: "Everyone is paid what they ask" and nothing changes. Result: "Raised 5 staff: +$184/day". It is a recurring salary change, not a one-off charge: cash and the ledger are untouched at the moment of the raise, salaries are still paid at the day close like any other. **Hard Bargain** (requires Payroll Day, level 7): Raise all instead pays only 60% of each gap (`new salary = salary + round(0.6 * (ask - salary))`), and the staff member takes it as a full raise, their ask drops to match the new salary, so they are satisfied and no follow-up request comes from that gap. Result: "Raised 5 staff: +$110/day (saved $74/day)".
...
- **Salary inflation** (v4, DESIGN 11.5): on Standard (1.5%) and Veteran (2.5%) every ask rises that much every 10th working day, for the first 12 periods only (prices never inflate, so endless inflation would squeeze every office to death). New candidates ask the same inflated rate; the raise rules above then apply. A skill event or a finished course recomputes the ask with the inflation factor. Relaxed never inflates.
...
- **Rent growth** (v4, DESIGN 11.5): `rent = office rent * (1 + min(rentGrowthCap, rentGrowthPerWeek * weeks at this tier * clamp(rating - 3, 0, 1.5)))`, weeks = working days since the location opened or moved / 5. Standard +8% a week at a 4.0 rating, cap +50%; Veteran +10%, cap +80%; Relaxed none. A move starts it over. `sim.rent(state, i)` is today's rent.
...
costs    = salaries + rent (with growth) + supplies*(1 - leanOps 0.2) + marketing + loan interest + loan payment + overdraft interest
           loan interest = 0.25% (Investor Relations 0.125%) of (principal + the emergency part again): an emergency loan costs double
net      = revenue - costs
```
Overdraft: while cash is below zero after the day's costs, 0.5% of the negative balance is charged as overdraft interest, the bank skips the loan's auto-payment (interest still runs), staff lose 8 morale a day, and Debt Free is only awarded with cash at or above zero. v4 adds **bankruptcy** (DESIGN 11.5): `state.distress` counts closes in a row below zero; from 2 a warning note ("Warning: cash has been below zero for 2 closes..."); at `bankruptcyDays` (Standard 5, Veteran 4, Relaxed never) the bank sells equipment at 50%, newest first, until cash is back at zero, then closes the newest location (half its office price back) if you own more than one, then forces an emergency loan (overdraft + $500, rounded up to $100) at double interest; distress starts over. After the costs the close also runs the Smile Vans, the city decay and the milestones (DESIGN 11.1). City grants, the gala prize, bank sales, emergency loans and Head Start are non-operating (`NON_OPERATING_LABELS`). `closeDay` returns the report with `events` (achievements, level-ups, quits) for the UI to emit after it; `operatingNet` is the day's operating net.
...
v4: these are the Relaxed targets (Relaxed plays the v3 rules). The v4 run measures the median bot on Relaxed at T2 1.85 h, T3 3.04 h, second location 3.97 h, Floss Boss 6.60 h (the 7.42 h below came from the v3 code; the rap star VIPs, the city grants and the bot's slowly improving hands-on quality put it a little ahead), full table in 11.7.
...
## 11. End game: make Smile City smile (v4)

The owner asked "whats the end game here? how should we make the user feel accomplished? also the game feels a bit too easy now", rejected a rivalry finale, and chose "Make Smile City smile". The arc: hygienist, owner, then the whole city. A city-wide Smile Index is the one number that sums up the career, it visibly changes the world, and reaching 100% ends in a parade and an on-stage finale. Catalogs: `src/data/city.ts`, `src/data/difficulty.ts`, `src/data/legacy.ts`, `src/core/legacy.ts`.

### 11.1 Smile City and the Smile Index
- Six districts (`DISTRICTS`), each with a Smile Index 0..1 starting at `start` (the city starts at 18.7%). The city index is the population-weighted mean (populations 1,040 to 1,440 smile points, 7,440 in all). `state.city` holds each district's index and patients served.
- Every location belongs to a district: `Clinic.district`, chosen when you open the practice or a new location (the district picker shows each district's index and blurb; a district can hold several of your locations, but a second location in the same district adds only half as much). Moving an office keeps its district. Bright Smiles Dental is Downtown.
- **Gains:** every patient served at a location raises that district's index by `(quality - 0.5) * 2 / population * cityGainMult` (so a poor clean below 0.5 lowers it; the multiplier only scales gains: Relaxed 1.3, Standard 0.9, Veteran 0.85); 80% of a location's patients come from its own district and 20% from a random neighbour, credited there (picked from a hash of the patient id, so crediting never draws from the world RNG). A second location in a district that already has an older one adds half. A wait walkout costs -0.5 points, a comfort walkout -1. The Smile Van (equipment, $20,000, Main Street and up, for sale from 50%, one per location) serves the least-smiling district with no location (a second van the next one, the least-smiling overall once every district has a location): 6 free outreach cleanings a day at quality 0.8 at the close. Charity (host 10 points, donate 3), school day (book the class 8) and tooth fairy (rent the costume 5) add points to their location's district, times cityGainMult. Hands-on cleans count double (the owner's personal touch). Employed, the patients of your own chair count for Downtown (hands-on double, Quick clean single).
- **Gentle decay:** a district loses `0.004 * (index - start)` per day when fewer than 3 of its patients were served that day, so the city needs ongoing care and a far-flung district left alone drifts back.
- **Milestones** (`CITY_MILESTONES`, every 10% from 30): a celebration card with the milestone text, a timeline entry, a cash gift (the city grant: $500 per 10% reached times the biggest office's tierScale, so $1,500 at 30% from a Strip Mall Suite and $25,000 at 100% from Smile Tower) and the listed unlock (50%: Smile Van for sale; 70%: Golden Molar nomination; 90%: the gala is planned). 100%: the parade, then the gala is scheduled. A milestone counts from the percent the UI shows (one decimal, so 99.5% is 100%) and is never taken back. The sim checks after every tick, hands-on clean and close: a 'toast' SimEvent ("Smile City 50%: Halfway There"), a report note, a 'city' timeline entry and the grant.
- **Pacing:** the median bot reaches 100% city at about 8 to 10 h of play on Standard (Relaxed about 6 to 7 h, Veteran 11 to 13 h). The balance tool reports it. Measured (11.7): Standard 8.24 h, Relaxed 6.60 h, Veteran 11.18 h.

### 11.2 Feeling accomplished
- **Promotions:** every title change (Staff, Senior, Lead Hygienist, Practice Owner, Clinic Director, Dental Mogul, Floss Boss) gets a full-screen ceremony: the new title badge, confetti, a line from Dr. Ruth Canal, the `promotion` sound. Timeline entry.
- **Ribbon cutting:** opening a practice, opening a location or moving up an office plays a short ceremony in the diorama: camera sweep, balloons, a ribbon at the door (`prop_ribbon`), `ribbon_snip` and a small crowd cheer.
- **Trophy wall:** a wall in every owned office shows framed plaques (`plaque_frame`) for achievements, gold case masteries and city milestones, and the Golden Molar trophy (`trophy_golden_molar`) once won. The clinic view receives a summary through `ClinicView.setTrophies({ plaques, gold, milestones, goldenMolar })`.
- **City mood:** the diorama street brightens with the location's district index: more passers-by who smile, a billboard showing the city percentage from 50%, confetti bits after milestones. `ClinicView.setCityMood(districtIndex, cityIndex)`.
- **Smile City screen:** a stylised map of the six districts, each filled from grey to its colour by its index, your locations as pins, the city percentage, the next milestone and what it unlocks. Reachable from the hub nav (owner phase; read-only preview in the employee phase showing Downtown rising from your cleans).
- **Legacy screen:** career stats (patients, chunks popped, perfect cleans, best combo, money earned, days), the timeline (`state.timeline`), case masteries, city progress. Always available from Goals; it is also the credits' final page.

### 11.3 The finale: the Golden Molar Gala
- At 100% city: the parade (full-screen celebration over the diorama), then a gala card: "The Golden Molar Gala is tonight. Headliner Lil Molar needs his grill show-ready." The gala runs as a hands-on **showcase clean** (the `grillz` case with `special.showcase = true`): the rap star (archetype `rapper`) on stage, a spotlight, a crowd meter that rises with pops, snaps and checklist ticks and falls with gum slips, all his twists, the grill with the most gems. 4+ stars wins; fewer lets you retry the next day (`finale.attempts`).
- Numbers (`sim.galaSetup`, `sim.completeGala`): 12 gems; chatty plus two seeded twists from hiccups, fidget, sensitive gums, sleepy and gag reflex (one on Relaxed); at least level-8 difficulty for the tooth count; comfort starts at 75; bonus "Snap every marked tooth"; no numbing gel; the rules of your title. One try a day (owner phase). **The crowd warms up:** every failed attempt lowers the gala's star thresholds by 0.015 (at most 0.12), so every player can win it (median bot: 1 try, casual 8, Veteran median 2). Winning pays the gala prize ($25,000 x the biggest office's tierScale, non-operating), writes the win to the Legacy store at once (wins, veteranUnlocked) and adds an 'award' timeline entry; a loss adds "Gala attempt N: 3 stars". The gala counts for stats and grill mastery like any hands-on clean and never ticks the clinic clock.
- Winning: the Golden Molar trophy, the credits (a scrolling list of the game's credits plus your staff and your best patients' names), the Legacy screen, then a choice: **Retire** (New Game+) or **Keep running the chain** (the game continues; the trophy stays on the wall).

### 11.4 Legacy and New Game+
- Retiring converts the run into Legacy points: 5 for the Golden Molar, 1 per 5 achievements, 1 per gold case mastery, 1 per $1M valuation (cap 10). `core/legacy.ts` stores them apart from the run save, so a new game never wipes them.
- The New Game screen shows the Legacy shop (`LEGACY_PERKS`, each bought once) and lets the player bring owned perks into the run (`state.legacyPerks`), plus the difficulty pick. The Golden Molar sets `veteranUnlocked` (Veteran is selectable from the start anyway; winning it adds a gold badge).
- As implemented: `sim.newGame({ ..., difficulty, legacyPerks })` applies Head Start ($500 through the ledger as 'Head Start', Floss Picks), Trained Hands (Gracey Curette, Cordless Polisher) and Prodigy (+1 skill point) at once; Famous Name is demand x1.1 at your locations; Alumni Network gives every new candidate +5 skill, speed and bedside at the same ask; Gold Scrubs is cosmetic. `sim.legacyPoints(state)` shows the breakdown, `sim.retire(state)` adds the points to `core/legacy` (points, earned, runs) once and marks `finale.retired`.

### 11.5 Difficulty ("too easy now")
`state.difficulty` (`DIFFICULTIES`), picked at new game, Standard by default. Standard is harder than v3:
- **Clean:** star thresholds rise with the title (`starShiftPerTitle`, capped), 5 stars also needs time within `fiveStarPar * par`, the snap point is `snapAt`, dirt is scaled by `dirtScale`, passive comfort drain grows past level 3, and up to `maxTwists` twists from level 8. The chair card and the case intro say what 5 stars needs ("5 stars: 94% and under 1:20").
- **Economy:** rent grows with a location's reputation (`rentGrowthPerWeek`, capped), costly events are more likely at big offices (`badEventWeight`), salary asks inflate slowly, and cash below zero has teeth: after 2 closes a warning, after `bankruptcyDays` consecutive closes below zero the bank acts (sells the newest equipment at 50%, then closes the newest location if that is not enough; with one location it forces an emergency loan at double interest). `state.distress` counts the closes.
- **Relaxed** keeps the v3 feel (forgiving stars, no bankruptcy). **Veteran** turns everything up.
- Balance targets: on Standard the median bot averages 3.5 to 4 stars on hands-on cleans at its level (5 stars needs skill), owner milestones take about 20% longer than v3 (T2 2 to 2.5 h), and at least one bot run in five meets the bankruptcy warning and recovers.

| | Relaxed | Standard | Veteran |
|---|---|---|---|
| snap point (`snapAt`) | 0.8 (v3) | 0.8 | 0.88 |
| star shift per title above Staff Hygienist, cap | 0 | 0.01, 0.03 | 0.015, 0.05 |
| 5 stars also needs time <= | none | 1.15 x par | 1.0 x par |
| dirt (`dirtScale`) | 1.0 (v3) | 1.1 | 1.3 |
| comfort drain past level 3, cap | none | +5% a level, x1.4 | +8% a level, x1.6 |
| twists from level 8 | 2 (v3) | 3 | 3 |
| rent growth a week, cap | none | 8%, 50% | 10%, 80% |
| costly events at a Medical Plaza or bigger | x1 (v3) | x1.5 | x2 |
| salary inflation every 10 days (12 periods) | none | 1.5% | 2.5% |
| bank acts after closes below zero | never | 5 | 4 |
| Smile Index gains | x1.3 | x0.9 | x0.85 |

- **Stars** (`CleanSetup.rules`, the clean scores with them): the 5-star and 4-star thresholds rise by `starShift` (0.92 and 0.80 plus the shift; 5 stars never above 0.99, 4 never above 5); 3 stars (0.65) and below never move, so case mastery and Quick clean stay in reach for every player. 5 stars also needs `seconds <= fiveStarPar * parSeconds`. `starShift = min(cap, perTitle * titles above Staff Hygienist)`: Senior 1, Lead 2, Practice Owner 3, Clinic Director 4, Dental Mogul 5, Floss Boss 6. School practicals: shift 0, no time limit. `sim.fiveStarNeeds(state, patientId)` gives the chair card its "5 stars: 95% and under 1:37".
- **Dirt:** tartar count, plaque, stain and debris x `dirtScale` on top of the level scaling (plaque capped at 0.95), outside school; NPC chair time x `sqrt(dirtScale)` at your offices (8.1). **Comfort:** `traits.comfortDrain x min(cap, 1 + perLevel * (level - 3))`. **Twists:** at most 1 below level 4, 2 from 4, `max(2, maxTwists)` from level 8.
- **Costly events** (the ones that cost whatever you answer): inspector, flu, burst pipe, rival, poaching, outage, heatwave.
- Titles never go back down in the owner phase (a closed location keeps your title), and every promotion adds a 'career' timeline entry and a report note "Promoted: Senior Hygienist" (a promotion at the close lands in the next report).

### 11.6 The rap star VIP
Archetype `rapper` (stage names, big tips, review weight 5, low patience), case `grillz` (Grill Glow-Up: hold on the grill 0.6 s to pop it out onto a tray, clean the heavier plaque and all of the case's tartar hidden under it, the grill flies back in once every tooth under it snaps, then buff each diamond with the polisher (about 0.8 s a gem, rising bling pitch), a fifth objective "Clean the other marked teeth" when marked teeth sit outside the grill, then rinse and suction; no food debris). The gala showcase adds a dark stage, a warm spotlight, curtains and a crowd meter (up on pops, snaps, ticks, gems and the grill coming out; down on slips, gags and zings; cheers at 50, 75 and 95%). A rare special guest from level 4, the headliner of the Celebrity Walk-in event and one of the Smile Studio's VIPs. In the diorama he has dreadlocks, a chain and sunglasses. Always a VIP (`DayPatient.vip`).

Numbers: arrival weight 0 at a Strip Mall Suite, 1 at Main Street (Bright Smiles is one), 2 at a Medical Plaza, 3 at Smile Tower, out of about 130 (1 to 2% of patients), never below level 4; the grill case is introduced once in the first shift at level 4 with him. He always brings the grill case (payMult 2.5: the employee wage and, at your offices, the cleaning fee), seats first, reviews at weight 5 and has no last name. The Celebrity Walk-in books him at a flat $900; the Smile Studio's daily VIP is him on about a third of days ($2,400), the influencer otherwise. Grill setup: the upper front six (teeth 4 to 9) are always marked, the rest of `problemToothCount(L)` spread over the other teeth; tartar 2 to 4 x 1.1, plaque 0.8, stain 0.3, debris 1 (then the difficulty's dirtScale); `grillGems` 6 to 10 (12 at the gala); par extra 20 s plus 2.5 s per gem; bonus from no slips, under par, snap every tooth (and a combo with 4+ deposits).

### 11.7 Sim rules as implemented and the v4 balance run

API (`src/sim/index.ts`): `newGame({ ..., difficulty, legacyPerks })`, `openPractice(state, { name, loan, district? })`, `openLocation(state, tier, name, loan, district?)` (default: the least-smiling district without a location), `cityStatus`, `galaStatus`, `galaSetup`, `completeGala`, `legacyPoints`, `retire`, `trophies`, `cleanRules`, `fiveStarNeeds`, `rent`. `migrate` gives older saves Standard, the city at its start values, the employer Downtown and every location a district of its own (the least-smiling free one), a timeline rebuilt from achievements (graduate, senior, lead, owner, t2 to t4, chain2, chain5) and gold masteries, the default finale, no Legacy perks and no distress.

Last `npm run balance` (5 seeds; every bot on Standard, the median bot also on Relaxed and Veteran; runs end when the gala is won; full output in `out/sim/balance-v4.txt`):

| Milestone | Relaxed target | Standard target | Median Std | Median Relaxed | Median Veteran | Casual Std | Expert Std | Greedy Std | No manager Std | Idle owner Std |
|---|---|---|---|---|---|---|---|---|---|---|
| Practice opened | 25-35 min | 25-40 min | 28.5 min (cleaning 15) | 27.1 | 33.9 | 37.1 (20) | 28.5 | 28.5 | 28.5 | 28.5 |
| Office T2 | 1.25-2 h | 2-2.5 h | 2.01 h | 1.85 | 2.60 | 2.69 | 1.96 | 1.54 | 2.16 | 0.74 |
| Office T3 | 2.5-3.5 h | 3-4.2 h | 3.41 h | 3.04 | 4.65 | 4.70 | 3.37 | 2.34 | 4.61 | 1.19 |
| Second location | 3.5-4.5 h | 4.2-5.4 h | 4.75 h | 3.97 | 6.65 | 6.11 | 4.79 | 2.94 | 6.88 | 1.83 |
| Floss Boss title | 6.5-10 h (v3: 7-10) | 7.5-12 h (about 1.2x Relaxed) | 7.95 h | 6.60 | 10.76 | 9.60 | 8.08 | after the gala | after the gala | after the gala |
| Smile City 30 / 50 / 70 / 90% | | | 3.88 / 5.83 / 6.72 / 7.59 h | 3.25 / 4.79 / 5.52 / 6.19 | 5.42 / 7.86 / 9.08 / 10.42 | 5.21 / 7.20 / 8.24 / 9.16 | 3.94 / 5.78 / 6.75 / 7.62 | 3.12 / 4.41 / 5.59 / 6.98 | 4.60 / 7.89 / 9.15 / 10.11 | 1.17 / 2.04 / 2.67 / 3.09 |
| Smile City 100% | 6-7 h | 8-10 h (Veteran 11-13) | 8.24 h | 6.60 | 11.18 | 9.78 | 8.30 | 7.88 | 10.80 | 3.77 |
| Golden Molar won (tries) | | | 8.32 h (1) | 6.66 (1) | 11.32 (2) | 10.53 (8) | 8.35 (1) | 7.94 (1) | 10.95 (2) | 3.82 (2) |
| Hands-on stars, share of 5 | | 3.5-4 | 3.60, 0% | 4.00, 10% | 3.07, 0% | 2.72, 0% | 4.38, 38% | 3.62, 1% | 3.61, 1% | 3.23 |

- Standard against Relaxed (the v3 rules): T2 +9%, T3 +12%, second location +20%, Floss Boss +20%, Smile City 100% +25%. The owner days to each milestone barely move (T2 day 15 vs 14, T3 35 vs 32); the difference is thinner margins later (operating net over owner days 61 to 120: $10,791 a day vs $14,996) and longer cleans.
- Bankruptcy: 2 of 5 greedy-expander runs (it keeps a tenth of a day of costs and no cushion after a purchase) meet the warning and recover without the bank acting; no other bot goes below zero.
- The greedy expander, the unmanaged median and the idle owner win the gala before they reach Floss Boss: the city is the finish line now. The idle owner (auto-huddle, autopilot, 4x clock) reaches 100% in 3.8 h of real time, the same tension as in 10.8.
- Checks (3 seeds, Standard): cash always equals the ledger and the loan round trip moves no money; T1 second operatory plus a hygienist pays back in 3 to 13 days (+$199/day); price at T1 1.0 / 1.2 / 1.5 nets $1,105 / $1,127 / $535 a day, at T2 $4,428 / $2,073 / $213; the 75% salary floor for 30 days at T2 loses 4 to 5 people and $822 to $1,798 a day; the insurance network nets $3,746 / $4,434 / $3,636 vs $3,536 / $4,285 / $3,297 independent; the richest event choice every time nets $2,663 / $4,439 / $2,870 vs $3,340 / $3,906 / $2,934 for the first choice; raise requests at a new T2 office: 0.39 a day approving each (inflation adds a few asks), 0.40 ignoring them, 0 with auto raises or an Office Manager. Campaigns at a full Standard office are noisy and weaker than in v3 (Kids Week -$1,478 / -$304 / +$216 a day; with a fresh spare operatory -$1,739 / -$2,132 / +$2,060; Smile Makeover with a lamp -$1,399 / +$2,161 / +$1,699).
