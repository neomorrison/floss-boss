...
Archetype `rapper` (stage names, big tips, review weight 5, low patience), case `grillz` (Grill Glow-Up: hold the grill to pop it out, clean the gunk underneath, buff each diamond until it glints, rinse and suction, the grill snaps back in with a bling finale). A rare special guest from level 4, the headliner of the Celebrity Walk-in event and one of the Smile Studio's VIPs. In the diorama he has dreadlocks, a chain and sunglasses. Always a VIP (`DayPatient.vip`).
